...
class DoctorsPage(customtkinter.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        customtkinter.CTkButton(photo_frame, text="Upload Photo", command=self.upload_photo, font=self.default_font).pack(pady=5)
    ...
    def add_doctor(self):
        ...
        if not name or not spec or not contact or not price:
    ...
    def on_row_select(self, event):
        ...
        if selected_item:
            ...
            if doc:
                ...
                self.name_entry.delete(0, 'end'); self.name_entry.insert(0, doc["name"])
                self.specialization_entry.delete(0, 'end'); self.specialization_entry.insert(0, doc["specialization"])
                self.contact_entry.delete(0, 'end'); self.contact_entry.insert(0, doc["contact"])
                self.price_entry.delete(0, 'end'); self.price_entry.insert(0, doc.get("price", ""))
